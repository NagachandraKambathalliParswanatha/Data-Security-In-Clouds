		...
		try {
			...
			if (access >= 1) {
				...
				for (Patient patient : patients) {
					System.out.println(patient+" ;;;;;;;;dfdfd");
					...
					decryptedPatient.setName(response.getFirstHeader("name")
							.getValue());
					decryptedPatient.setGender(response
							.getFirstHeader("gender").getValue());
					decryptedPatient.setMobile(response
							.getFirstHeader("mobile").getValue());
					decryptedPatient.setAge(response.getFirstHeader("age")
							.getValue());
					decryptedPatient.setAddress(response.getFirstHeader(
							"address").getValue());
				...
				resp.sendRedirect("patientsList.jsp?access=no");
			}
